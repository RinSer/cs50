...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    *length = 0;
    ...
    while (true)
    {
        ...
        if (bytes > 0)
        {
            // Reallocate memory for content and copy the buffer into it
            *content = realloc(*content, *length+bytes);
            memcpy(*content+*length, buffer, bytes);
            // Increment length by the number of bytes in the buffer
            *length += bytes;
        }
        else
        {
            // Stop reading file if no bytes are left
            break;
        }
    }
...
const char* lookup(const char* path)
{
    ...
    {
        if (strcasecmp(".css", type) == 0)
        {
            return "text/css";
        }
        else if (strcasecmp(".html", type) == 0)
        {
            return "text/html";
        }
        else if (strcasecmp(".js", type) == 0)
        {
            return "text/javascript";
        }
        else if (strcasecmp(".php", type) == 0)
        {
            return "text/x-php";
        }
        else if (strcasecmp(".jpg", type) == 0)
        {
            return "image/jpeg";
        }
        else if (strcasecmp(".gif", type) == 0)
        {
            return "image/gif";
        }
        else if (strcasecmp(".png", type) == 0)
        {
            return "image/png";
        }
        else if (strcasecmp(".ico", type) == 0)
        {
            return "image/x-icon";
        }
        else
...
bool parse(const char* line, char* abs_path, char* query)
{
    if (line != NULL && strstr(line, "\r\n") != NULL)
    {
        ...
        char* http_version = strrchr(request, ' ');
        char* crlf = strchr(request, '\r');
        *crlf = 0;
        ...
        else if(strcmp(http_version+1, "HTTP/1.1") != 0)
        {
            free(request);
            error(505);
            return false;
        ...
        {
            *http_version = 0;
            char* request_query = strchr(path, '?');
...
/**
 * Returns status code's reason p
 * hrase.
 *
