/**
 * copy.c
 *
 * Computer Science 50
 * Problem Set 4
 *
 * Resizes a BMP picture.
 */
       
#include <stdio.h>
#include <stdlib.h>

#include "bmp.h"

int main(int argc, char* argv[])
{
    // ensure proper usage
    if (argc != 4)
    {
        printf("Usage: ./resize n infile outfile\n");
        return 1;
    }
    
    // value to resize
    float n = atof(argv[1]);
    if (n <= 0 || n > 100)
    {
        printf("The resize value is invalid\n");
        return 1;
    }

    // remember filenames
    char* infile = argv[2];
    char* outfile = argv[3];

    // open input file 
    FILE* inptr = fopen(infile, "r");
    if (inptr == NULL)
    {
        printf("Could not open %s.\n", infile);
        return 2;
    }

    // open output file
    FILE* outptr = fopen(outfile, "w");
    if (outptr == NULL)
    {
        fclose(inptr);
        fprintf(stderr, "Could not create %s.\n", outfile);
        return 3;
    }

    // read infile's BITMAPFILEHEADER
    BITMAPFILEHEADER bf;
    fread(&bf, sizeof(BITMAPFILEHEADER), 1, inptr);

    // read infile's BITMAPINFOHEADER
    BITMAPINFOHEADER bi;
    fread(&bi, sizeof(BITMAPINFOHEADER), 1, inptr);

    // ensure infile is (likely) a 24-bit uncompressed BMP 4.0
    if (bf.bfType != 0x4d42 || bf.bfOffBits != 54 || bi.biSize != 40 || 
        bi.biBitCount != 24 || bi.biCompression != 0)
    {
        fclose(outptr);
        fclose(inptr);
        fprintf(stderr, "Unsupported file format.\n");
        return 4;
    }
    
    // determine padding for scanlines
    int padding =  (4 - (bi.biWidth * sizeof(RGBTRIPLE)) % 4) % 4;
    
    // create resized pic BITMAPFILEHEADER
    BITMAPFILEHEADER new_bf;
    new_bf = bf;
    
    // create resized pic BITMAPINFOHEADER
    BITMAPINFOHEADER new_bi;
    new_bi = bi;
    
    // Count resized width and height for Info Header
    new_bi.biWidth = bi.biWidth*n;
    new_bi.biHeight = bi.biHeight*n;
    
    // determine resized file padding
    int new_padding =  (4 - (new_bi.biWidth * sizeof(RGBTRIPLE)) % 4) % 4;
    
    // Count resized pic size for Info Header
    new_bi.biSizeImage = (new_bi.biWidth*sizeof(RGBTRIPLE)+new_padding)*abs(new_bi.biHeight);
    
    // Count resized file size for File Header
    new_bf.bfSize = new_bi.biSizeImage + sizeof(BITMAPFILEHEADER) + sizeof(BITMAPINFOHEADER);

    // write outfile's BITMAPFILEHEADER
    fwrite(&new_bf, sizeof(BITMAPFILEHEADER), 1, outptr);

    // write outfile's BITMAPINFOHEADER
    fwrite(&new_bi, sizeof(BITMAPINFOHEADER), 1, outptr);

    // iterate over infile's scanlines
    if (bi.biWidth <= new_bi.biWidth)
    {
        for (int i = 0, biHeight = abs(bi.biHeight); i < biHeight; i++)
        {
            // write each scanline n times
            for (int h = 0; h < n; h++)
            {
                // iterate over pixels in scanline
                for (int j = 0; j < bi.biWidth; j++)
                {
                    // temporary storage
                    RGBTRIPLE triple;
            
                    // read RGB triple from infile
                    fread(&triple, sizeof(RGBTRIPLE), 1, inptr);
            
                    // write RGB triple to outfile n times
                    for (int w = 0; w < n; w++)
                    {
                        fwrite(&triple, sizeof(RGBTRIPLE), 1, outptr);
                    }
                }
                    
                // skip over padding, if any
                fseek(inptr, padding, SEEK_CUR);
            
                // then add it back (to demonstrate how)
                for (int p = 0; p < new_padding; p++)
                {
                    fputc(0x00, outptr);
                }
                    
                // Return back to the scanline beginning
                if (h < n-1)
                {
                    fseek(inptr, -(bi.biWidth*sizeof(RGBTRIPLE)+padding), SEEK_CUR);
                }
            }
        }
    }
    else
    {
        for (int i = 0, biHeight = abs(new_bi.biHeight); i < biHeight; i++)
        {
            // iterate over pixels in trimmed scanline
            for (int j = 0; j < new_bi.biWidth; j++)
            {
                // temporary storage
                RGBTRIPLE triple;
        
                // read RGB triple from infile
                fread(&triple, sizeof(RGBTRIPLE), 1, inptr);
        
                // write RGB triple to outfile n times
                fwrite(&triple, sizeof(RGBTRIPLE), 1, outptr);
                    
                // Seek point after several
                fseek(inptr, sizeof(RGBTRIPLE)*(1/n-1), SEEK_CUR);
            }
                
            // Add padding to the new file
            for (int p = 0; p < new_padding; p++)
            {
                fputc(0x00, outptr);
            }
                
            // Skip initial file padding
            fseek(inptr, padding, SEEK_CUR);
            
            // Skip initial file lines
            for (int s = 0; s < 1/n-1; s++)
            {
                fseek(inptr, sizeof(RGBTRIPLE)*bi.biWidth+padding, SEEK_CUR);
            }
        }
    }

    // close infile
    fclose(inptr);

    // close outfile
    fclose(outptr);

    // that's all folks
    return 0;
}
